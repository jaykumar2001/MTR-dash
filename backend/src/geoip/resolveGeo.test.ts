...
describe('resolveGeo', () => {
  ...
  it('returns the MaxMind result when it has a country', async () => {
    ...
    expect(result).toEqual({ country: 'US', city: 'Mountain View', source: 'maxmind' });
  ...
  it('falls back to ipdeny country (with null city) when MaxMind returns null', async () => {
    ...
    expect(result).toEqual({ country: 'DE', city: null, source: 'fallback' });
  });
  ...
  it('returns all-null when both MaxMind and ipdeny have nothing', async () => {
    ...
    expect(result).toEqual({ country: null, city: null, source: 'fallback' });
  });
});
